...
| 구분 | 토큰명 | 값 | 용도 |
|------|--------|-----|------|
| 배경 | `--calm-white` | `#FAFAF9` | 페이지 배경 |
| 카드 | `--calm-cloud` | `#F4F4F2` | 카드/섹션 배경 |
| 보더 | `--calm-stone` | `#E8E8E5` | 구분선, 테두리 |
| 보조텍스트 | `--calm-ash` | `#B8B8B3` | 라벨, 보조 텍스트 |
| 본문텍스트 | `--calm-charcoal` | `#52524E` | 본문 텍스트 |
| 헤딩 | `--calm-obsidian` | `#1A1A17` | 제목, 강조 |
| **악센트** | `--calm-accent` | `#3B82F6` | **Primary Blue** |
| 악센트라이트 | `--calm-accent-light` | `#DBEAFE` | 호버, 배경 강조 |
| 악센트다크 | `--calm-accent-dark` | `#1E40AF` | 클릭, 활성 상태 |

**디자인 철학**: Korean Luxury Minimalism (차분한 통제)
- 90% Neutrals + 10% Blue Accent
- Light Theme 기반
- "Show, Don't Tell" 원칙
...
```
공식 시스템:  Light Theme + Blue Accent (#3B82F6)
문제 컴포넌트: Dark Theme + Gold Accent (#d4af37)
```
...
```css
/* 제거 대상 (Gold Theme) */
#d4af37, #f4d9b0, #c0a36b, #f4e5c2  /* Gold variants */
...
```bash
# CSS 변수 사용 현황
grep -roh "var(--calm-[^)]*)" web/src/ | sort | uniq -c | sort -rn
```
...
module.exports = {
  rules: {
    ...
    'declaration-property-value-allowed-list': {
      'color': ['/^var\\(--calm-/', '/^transparent/', '/^inherit/'],
      'background': ['/^var\\(--calm-/', '/^transparent/', '/^linear-gradient/'],
      'background-color': ['/^var\\(--calm-/', '/^transparent/'],
      'border-color': ['/^var\\(--calm-/', '/^transparent/'],
    },
  }
};
```
...
| 작업 | 파일 | 변경 내용 |
|------|------|----------|
| 1.1 | `BookingModal.tsx` | light theme + blue accent 전환 |
| 1.2 | `LoadingSpinner.tsx` | calm-white 배경, calm-accent 스피너 |
| 1.3 | `StatCard.tsx` | calm-accent 기본값 적용 |
...
# 3. 디자인 토큰 사용률 체크
grep -roh "var(--calm-[^)]*)" web/src/ | wc -l
```
