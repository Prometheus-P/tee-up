...
describe('TERMS constants', () => {
  ...
  describe('TERMS immutability', () => {
    it('should be a readonly object', () => {
      ...
      expect(Object.isFrozen(TERMS)).toBe(false); // Objects marked with `as const` aren't frozen at runtime
...
describe('Type exports', () => {
  it('should allow TermKey to be assigned valid keys', () => {
    const key: TermKey = 'EXPERT_TITLE';
    expect(key).toBe('EXPERT_TITLE');
  });

  it('should allow TermValue to be assigned valid values', () => {
    const value: TermValue = TERMS.EXPERT_TITLE;
    expect(value).toBe('프로');
  });

  it('should enforce type safety for TERMS access', () => {
    // This test verifies that accessing TERMS with a valid key returns the expected type
    const expertTitle: typeof TERMS.EXPERT_TITLE = TERMS.EXPERT_TITLE;
    expect(expertTitle).toBe('프로');
  });
});
